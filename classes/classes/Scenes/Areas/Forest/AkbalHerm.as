package classes.Scenes.Areas.Forest
{
	...
	public class AkbalHerm extends Monster
	{
		...
		override public function eAttack():void
		{
			...
			if (findStatusEffect(StatusEffects.Blind) >= 0) {
				outputText(capitalA + short + " seems to have no problem guiding her attacks towards you, despite her blindness.\n", false);
			}
			//Determine if dodged!
			...
			if (rand(2) == 0) {
				...
				if (damage <= 0) {
					...
					if (player.armorDef > 0)
						outputText(" and " + player.armorName + ", she fails to deal any damage.", false);
					else
						outputText(" she fails to deal any damage.", false);
				}
				else {
					outputText("Akbal rushes at you, her claws like lightning as they leave four red-hot lines of pain across your stomach.", false);
				...
				if (damage == 0) {
					...
					outputText("Akbal snarls as she flies towards you, snapping her ivory teeth on your arm. You scream out in pain as you throw her off.", false);
					player.takeDamage(damage);
				}
			}
			game.combat.combatRoundOver();
		...
		{
			game.forest.akbalHermScene.akbalDefeated(hpVictory);
		}

		override public function won(hpVictory:Boolean, pcCameWorms:Boolean):void
		{
			game.forest.akbalHermScene.akbalWon(hpVictory,pcCameWorms);
		...
		public function akbalLustAttack():void
		{
			...
			{
				outputText("You hear whispering in your head. Akbal begins speaking to you as she circles you, telling all the ways she'll dominate you once she beats the fight out of you.", false);
				//(Lust increase)
		...
		{
			if (HPRatio() >= 1)
				outputText("Akbal licks herself, ignoring you for now.", false);
			else
				outputText("Akbal licks one of her wounds, and you scowl as the injury quickly heals itself.", false);
			addHP(30);
			lust += 10;
			game.combat.combatRoundOver();
		}

		public function AkbalHerm()
		{
			trace("AkbalHerm Constructor!");
			this.a = "";
			this.short = "Akbal";
			this.imageName = "akbal";
			this.long = "Akbal, 'God of the Terrestrial Fire', circles around you. Her sleek yet muscular body is covered in tan fur, with dark spots that seem to dance around as you look upon them.  Her mouth holds two ivory incisors that glint in the sparse sunlight as her lips tremble to the sound of an unending growl.  Each paw conceals lethal claws capable of shredding men and demons to ribbons.  Her large and sickeningly alluring bright green eyes promise unbearable agony as you look upon them.";
			// this.plural = false;
